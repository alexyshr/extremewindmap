...
ruta="./../data/"
#ruta="D:/jd/01 C?digos de programaci?n/02 Ajuste con R/alexys/"
...
fn <- "fitted_model_result_PoissonProcessGumbelIntFunc.xlsx"
if (file.exists(fn))
...
fnfitted <- paste0(ruta, "raw_data_station_", number, "_fitted", ".xlsx")
if (file.exists(fnfitted))
  #Delete file if it exists
  file.remove(fnfitted)

raw.data <- ReadWindFile(station.number=number, path = ruta)
library(dplyr)
...
source('statistics_raw_data_nt.r')
...
source('statistics_raw_data_t.r')
...
source('write_t_nt_csv_one_data_per_day.r')
...
source('statistics_and_graphics_declustered_nt.r')

#Statistics and graphics for declustered thunderstorm
source('statistics_and_graphics_declustered_t.r')
