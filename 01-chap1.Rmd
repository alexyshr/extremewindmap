...
big = ggplot(data = world) + 
  ...
  scale_color_manual(values = c("Stations" = "black"), name="ERA5", guide = guide_legend(override.aes = list(fill= c(NA), linetype = c("blank"), shape = c(".")))) +
...
geom_rect(mapping=aes(xmin=-79.252968100, xmax=-78.247031900, ymin=11.832846362, ymax=12.667153638), color="red", alpha=0, size=0.1) +
  ...
  annotate(geom = "text", x = -80, y = 5, label = "Pacific\nSea", fontface = "italic", color = "grey22", size = 2) +
  geom_text_repel(data = era5_4326_sf_pol_filter_corners_left, size=2, aes(x=x, y=y, label = DN), direction="y", segment.size=0.1, segment.color= "grey50", color="grey50", nudge_x=-1, hjust=1, box.padding=0.1) +
  geom_text_repel(data = era5_4326_sf_pol_filter_corners_right, size=2, aes(x=x, y=y, label = DN), direction="y", segment.size=0.1, segment.color= "grey50", color="grey50", nudge_x=+1, hjust=0, box.padding=0.1) +
#  geom_stars(data = era5colraster.st) + #, size=1, shape=2, color= "black", show.legend = "point") + 
  #annotation_scale(location = "bl", width_hint = 0.5) + annotation_north_arrow(location = "br", which_north = "true", pad_x = unit(0.02, "in"), pad_y = unit(0.02, "in"), height = unit(1, "cm"), width = unit(1, "cm"), style = north_arrow_fancy_orienteering) + 
...
corner1lt = ggplot(data = world) + 
  ...
  #theme(axis.text.x = element_text(margin =  margin(t =0, b = -20))) + 
  #theme(axis.text.y = element_text(margin =  margin(r =0, l = -40))) +
...
#lay <- rbind(c(1,1,1,2,3),c(1,1,1,4,5))

#gs <- lapply(1:5, function(ii) 
#  grobTree(rectGrob(gp=gpar(fill=NA))))

# plot_grid(
#   # row 1
#   plot_grid(big, nrow = 1, labels = c('A')) +
#     theme(plot.background = element_rect(color = "black")),
# 
#   # row 2
#   plot_grid(corner1lt, corner2rt, corner3lb, corner4rb, nrow = 2, ncol=2, labels = c('B', 'C', 'D', 'E')) +
#     theme(plot.background = element_rect(color = "black")), 
# 
#   ncol = 2)

#grid.arrange(big, corner1lt, corner2rt, corner3lb, corner4rb, layout_matrix = lay)

grid.arrange(big, arrangeGrob(corner1lt, corner2rt, corner3lb, corner4rb), ncol=2, widths=c(2.2,1))
#grid.arrange(big, arrangeGrob(corner1lt, corner2rt, corner3lb, corner4rb, ncol=2, heights=c(1, 1), widths=c(1,1)), ncol=2, widths=c(1.7,1), heights=c(1))#, grobs = lapply(pl, "+", margin))
