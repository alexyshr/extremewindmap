...
timestamp_isdlite <- as.POSIXct(as_tibble(select(isdlite, mydatetime))$mydatetime,format="%Y-%m-%d %H", tz="UTC")
library(xts)
...
#str(timestamp_string)
  statera5_xts = na.omit(xts(x= ncvar_get(ncin, 'fg10', start=c(as.integer(era5_intersect_ideam["ideam_lon_index",i]),as.integer(era5_intersect_ideam["ideam_lat_index",i]),1), count=c(1,1,ntime)), order.by = timestamp))
  colnames(statera5_xts) = paste0("ERA5_", era5_intersect_ideam["ideam_station",i])
