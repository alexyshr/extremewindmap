...
kable(mytable_latex, linesep = "", align=rep('l', 2), format = "latex", escape = FALSE, #format = "markdown",
       ...
       caption.short = "Python Scripts to download ERA5 data",
       longtable = TRUE,
       booktabs = TRUE) %>%
       row_spec(0, align = "l") %>%
...
1. Use R version 3.6.2, RStudio Version 1.2.5033, and 61 R packages listed at the beginning of this manual. To proceed it is necessary to install all the packages.
...
Files to run POT-PP in ISD stations are inside the folder **./pot_pp/isd/**. Main file to run this process is **pot_pp_isd.r**, below are the main variables to configure within this file.


1. _inputpath="./raw_data/"_, 
...
First line must always have next content _"date_time" "mph" "thunder_flag"_, representing three column names, separated by spaces. First column _"date_time"_, has the date time in the format "YYYY/MM/DD HH:MM:SS", time zone UTC. Second column _kph_, must have the standardized wind velocity. Third column _"thunder_flag"_, must have a flag with one of two possible values _"nt"_ ot _"t"_, representing respectively _non-thunderstorm_ or _thunderstorm_ classification for the current wind value. For current research, third column always had the flag _"nt"_, for all rows, all stations. The separation of each column value must be _space_ character. Values corresponding to _"date_time"_ and _"thunder_flag"_ must be enclosed by double quotes.
...
Be aware that for each line in this file _01 estaciones.txt_, one text file with the time series, with the name **raw_data_station_\*.txt**, where _\*_ represents the same station ID, must be stored inside _inputpath_ folder. See first element, lines up, in this list of variables. 
...
Variable _outputpath_, should point to the folder where all output files will be stored, after running POT-PP process. See Annex \@ref(results) - Results - Digital Files, Table \ref{tab:resultsstructure}, with detailed list of output files (in this case inside **./potpp/isd/**). Files that will be generated inside _outputpath_ are displayed in next list, where _\*_ will be replaced by correspondent station ID.
