...
To download the data from CDS there are three main options: (1) Using the CDS web page <https://cds.climate.copernicus.eu/>, (2) using the `CDSAPI` with Python, and (3) using `ecmwfr` R package as interface to ECMWF and CDS API and Web Services [@Hufkens2019; @Hufkens2020].

Next block of code shows the use of `ecmwfr` R package to download ERA5 variable _10fg - 10 meters wind gust_, from 1979 to 1991 for Colombia area. The most important keywords there, allow to define ERA5 data source and product type, variable name _'variable'_, format NetCDF _'format'_, area of interest _'area'_, using WGS88 coordinates in the format _north, west, south, east_, cell size _'grid'_ in decimal degrees, and all the keywords related to time _'year'_, _'month'_, _'day'_, _'time'_. To be able to process the data request it is necessary to provide the user (a number) and key as specified below.

```{r results='hide', eval=FALSE, vspaceecho='0.4cm', echo = TRUE, size="scriptsize"}
  require(ecmwfr)
  
  #Change user and key
  wf_set_key(user = "26208", key = "*-*-*-*", service="cds")
  ...
  request <- list(
    ...
    area = c(12.5, -79.1, -4.5, -66.8),
    grid = c(0.25, 0.25),
    dataset_short_name = "reanalysis-era5-single-levels,",
    target = "10fg_1979_1991.nc"
  )
  
  tmp<-capture.output(pth<-wf_request(request))
  file.copy(pth, '10fg_1979_1991.nc', overwrite = TRUE)
```

To use the CDSAPI of Python to download the same previous dataset, use next block of code. The module `cdsapi` must be installed beforehand.
