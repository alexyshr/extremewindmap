...
%Uncomment next line to avoid hyperlinks in PDF file. Usefull to manage error:
%
%
%\hypersetup{draft}
\begin{document}
...
Next code, from ISD data source stored in unstacked PostgreSQL table \emph{isd\_lite\_unstack}, twelve stations are loaded (using WHERECLAUSE to filter), and roughness correction factor is applied. Listed ISD stations, are equivalent to previous IDEAM stations, reason why correction factor values are identical. An efficient use of memory is made here due to the use of lazy tibble dataframes from PostgreSQl database, see Annex \ref{dbstoring} - \protect\hyperlink{dbstoring}{Database Storing}.
...
\NormalTok{      filenames =}\StringTok{ }\KeywordTok{list.files}\NormalTok{(path_vv_aut_}\DecValTok{10}\NormalTok{, }\DataTypeTok{pattern =} \StringTok{"VV_AUT_10@"}\NormalTok{)}
    \CommentTok{#Variable to hold station IDs whitout roughness correction factor}
...
\NormalTok{          fexpo =}\StringTok{ }\NormalTok{fc}\OperatorTok{$}\NormalTok{fexpo[fc}\OperatorTok{$}\NormalTok{station_id }\OperatorTok{==}\StringTok{ }\KeywordTok{as.integer}\NormalTok{(station_id)]  }
        \CommentTok{#Keep the record of stations whitout correction factors in Excel file}
