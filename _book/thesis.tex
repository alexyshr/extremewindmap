...
A complete R tool was implemented to solve the whole process, which is based on copyrighted code for de-clustering and thresholding generously given by Dr Adam L. Pintar \href{mailto:adam.pintar@nist.gov}{\nolinkurl{adam.pintar@nist.gov}} - National Institute of Standards and Technology NIST, U.S Department of Commerce.
}
...
\begin{figure}
\centering
\includegraphics{thesis_files/figure-latex/winddatabase-1.pdf}
...
Hufkens, K. (2020). \emph{Ecmwfr: Interface to 'ecmwf' and 'cds' data web services}. Retrieved from \url{https://CRAN.R-project.org/package=ecmwfr}

\leavevmode\hypertarget{ref-Hufkens2019}{}%
Hufkens, K., Stauffer, R., \& Campitelli, E. (2019). The ecwmfr package: An interface to ecmwf api endpoints. \url{http://doi.org/10.5281/zenodo.2647541}
...
The European Center for Medium-Range Weather Forecasts - ECMWF had implemented the Climate Data Storage - CDS where all its datasets can be downloaded. It is necessary to research about names and meanings of ERA5 variables using the official \emph{data documentation} web page \url{https://confluence.ecmwf.int/display/CKB/ERA5\%3A+data+documentation}, or the \emph{parameter database} \url{https://apps.ecmwf.int/codes/grib/param-db}, that includes all ECWMF data sources, not only ERA5.

To download the data from CDS there are three main options: (1) Using the CDS web page \url{https://cds.climate.copernicus.eu/}, (2) using the \texttt{CDSAPI} with Python, and (3) using \texttt{ecmwfr} R package as interface to ECMWF and CDS API and Web Services (Hufkens, \protect\hyperlink{ref-Hufkens2020}{2020}; Hufkens, Stauffer, \& Campitelli, \protect\hyperlink{ref-Hufkens2019}{2019}).

Next block of code shows the use of \texttt{ecmwfr} R package to download ERA5 variable \emph{10fg - 10 meters wind gust}, from 1979 to 1991 for Colombia area. The most important keywords there, allow to define ERA5 data source and product type, variable name \emph{`variable'}, format NetCDF \emph{`format'}, area of interest \emph{`area'}, using WGS88 coordinates in the format \emph{north, west, south, east}, cell size \emph{`grid'} in decimal degrees, and all the keywords related to time \emph{`year'}, \emph{`month'}, \emph{`day'}, \emph{`time'}. To be able to process the data request it is necessary to provide the user (a number) and key as specified below.
...
\begin{Shaded}
\begin{Highlighting}[]
  \KeywordTok{require}\NormalTok{(ecmwfr)}
  
  \CommentTok{#Change user and key}
  \KeywordTok{wf_set_key}\NormalTok{(}\DataTypeTok{user =} \StringTok{"26208"}\NormalTok{, }\DataTypeTok{key =} \StringTok{"*-*-*-*"}\NormalTok{, }\DataTypeTok{service=}\StringTok{"cds"}\NormalTok{)}
  
\NormalTok{  request <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}
    \DataTypeTok{product_type =} \StringTok{"reanalysis"}\NormalTok{,}
    \DataTypeTok{format =} \StringTok{"netcdf"}\NormalTok{,}
    \DataTypeTok{variable =} \StringTok{"10m_wind_gust_since_previous_post_processing"}\NormalTok{,}
    \DataTypeTok{year =} \KeywordTok{c}\NormalTok{(}\StringTok{"1979"}\NormalTok{, }\StringTok{"1980"}\NormalTok{, }\StringTok{"1981"}\NormalTok{, }\StringTok{"1982"}\NormalTok{, }\StringTok{"1983"}\NormalTok{, }\StringTok{"1984"}\NormalTok{, }
             \StringTok{"1985"}\NormalTok{, }\StringTok{"1986"}\NormalTok{, }\StringTok{"1987"}\NormalTok{, }\StringTok{"1988"}\NormalTok{, }\StringTok{"1989"}\NormalTok{, }\StringTok{"1990"}\NormalTok{, }\StringTok{"1991"}\NormalTok{),}
    \DataTypeTok{month =} \KeywordTok{c}\NormalTok{(}\StringTok{"01"}\NormalTok{, }\StringTok{"02"}\NormalTok{, }\StringTok{"03"}\NormalTok{, }\StringTok{"04"}\NormalTok{, }\StringTok{"05"}\NormalTok{, }\StringTok{"06"}\NormalTok{, }\StringTok{"07"}\NormalTok{, }\StringTok{"08"}\NormalTok{, }\StringTok{"09"}\NormalTok{, }\StringTok{"10"}\NormalTok{, }\StringTok{"11"}\NormalTok{, }\StringTok{"12"}\NormalTok{),}
    \DataTypeTok{time =} \KeywordTok{c}\NormalTok{(}\StringTok{"00:00"}\NormalTok{, }\StringTok{"01:00"}\NormalTok{, }\StringTok{"02:00"}\NormalTok{, }\StringTok{"03:00"}\NormalTok{, }\StringTok{"04:00"}\NormalTok{, }\StringTok{"05:00"}\NormalTok{, }\StringTok{"06:00"}\NormalTok{, }\StringTok{"07:00"}\NormalTok{, }
             \StringTok{"08:00"}\NormalTok{, }\StringTok{"09:00"}\NormalTok{, }\StringTok{"10:00"}\NormalTok{, }\StringTok{"11:00"}\NormalTok{, }\StringTok{"12:00"}\NormalTok{, }\StringTok{"13:00"}\NormalTok{, }\StringTok{"14:00"}\NormalTok{, }\StringTok{"15:00"}\NormalTok{, }
             \StringTok{"16:00"}\NormalTok{, }\StringTok{"17:00"}\NormalTok{, }\StringTok{"18:00"}\NormalTok{, }\StringTok{"19:00"}\NormalTok{, }\StringTok{"20:00"}\NormalTok{, }\StringTok{"21:00"}\NormalTok{, }\StringTok{"22:00"}\NormalTok{, }\StringTok{"23:00"}\NormalTok{),}
    \DataTypeTok{day =} \KeywordTok{c}\NormalTok{(}\StringTok{"01"}\NormalTok{, }\StringTok{"02"}\NormalTok{, }\StringTok{"03"}\NormalTok{, }\StringTok{"04"}\NormalTok{, }\StringTok{"05"}\NormalTok{, }\StringTok{"06"}\NormalTok{, }\StringTok{"07"}\NormalTok{, }\StringTok{"08"}\NormalTok{, }\StringTok{"09"}\NormalTok{, }\StringTok{"10"}\NormalTok{, }
            \StringTok{"11"}\NormalTok{, }\StringTok{"12"}\NormalTok{, }\StringTok{"13"}\NormalTok{, }\StringTok{"14"}\NormalTok{, }\StringTok{"15"}\NormalTok{, }\StringTok{"16"}\NormalTok{, }\StringTok{"17"}\NormalTok{, }\StringTok{"18"}\NormalTok{, }\StringTok{"19"}\NormalTok{, }\StringTok{"20"}\NormalTok{, }
            \StringTok{"21"}\NormalTok{, }\StringTok{"22"}\NormalTok{, }\StringTok{"23"}\NormalTok{, }\StringTok{"24"}\NormalTok{, }\StringTok{"25"}\NormalTok{, }\StringTok{"26"}\NormalTok{, }\StringTok{"27"}\NormalTok{, }\StringTok{"28"}\NormalTok{, }\StringTok{"29"}\NormalTok{, }\StringTok{"30"}\NormalTok{, }\StringTok{"31"}\NormalTok{),}
    \DataTypeTok{area =} \KeywordTok{c}\NormalTok{(}\FloatTok{12.5}\NormalTok{, }\FloatTok{-79.1}\NormalTok{, }\FloatTok{-4.5}\NormalTok{, }\FloatTok{-66.8}\NormalTok{),}
    \DataTypeTok{grid =} \KeywordTok{c}\NormalTok{(}\FloatTok{0.25}\NormalTok{, }\FloatTok{0.25}\NormalTok{),}
    \DataTypeTok{dataset_short_name =} \StringTok{"reanalysis-era5-single-levels,"}\NormalTok{,}
    \DataTypeTok{target =} \StringTok{"10fg_1979_1991.nc"}
\NormalTok{  )}
  
\NormalTok{  tmp<-}\KeywordTok{capture.output}\NormalTok{(pth<-}\KeywordTok{wf_request}\NormalTok{(request))}
  \KeywordTok{file.copy}\NormalTok{(pth, }\StringTok{'10fg_1979_1991.nc'}\NormalTok{, }\DataTypeTok{overwrite =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
\normalsize

To use the CDSAPI of Python to download the same previous dataset, use next block of code. The module \texttt{cdsapi} must be installed beforehand.
...
\NormalTok{      c.retrieve(}\StringTok{'reanalysis-era5-single-levels'}\NormalTok{,\{}
      ...
      \StringTok{'10fg_1979_1991.nc'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
\normalsize
...
\caption[PostgreSQL Database Credentials]{\label{tab:unnamed-chunk-13}PostgreSQL Database Credentials}\\
\toprule
...
\CommentTok{# ncin <- nc_open(filename)}
\CommentTok{# lon <- ncvar_get(ncin, "longitude")}
\CommentTok{# nlon = dim(lon)}
\CommentTok{# lat <- ncvar_get(ncin, "latitude")}
\CommentTok{# nlat = dim(lat)}
\CommentTok{# ntime <-  dim(ncvar_get(ncin, "time"))}
\CommentTok{# variablename <- "fg10"}
\CommentTok{# fg10.units <- ncatt_get(ncin, variablename, "units")}
\CommentTok{# fg10.units}
\CommentTok{# lonlat.unstack <- expand.grid(lon=as.numeric(lon), lat=as.numeric(lat))}

\CommentTok{#Load using stars}
\NormalTok{lon <-}\StringTok{ }\KeywordTok{read_ncdf}\NormalTok{(filename, }\DataTypeTok{var =} \KeywordTok{c}\NormalTok{(}\StringTok{"longitude"}\NormalTok{))}
\NormalTok{nlon =}\StringTok{ }\KeywordTok{dim}\NormalTok{(lon)}
\NormalTok{lat <-}\StringTok{ }\KeywordTok{read_ncdf}\NormalTok{(filename, }\DataTypeTok{var =} \KeywordTok{c}\NormalTok{(}\StringTok{"latitude"}\NormalTok{))}
\NormalTok{nlat =}\StringTok{ }\KeywordTok{dim}\NormalTok{(lat)}
\NormalTok{ntime <-}\StringTok{  }\KeywordTok{dim}\NormalTok{(}\KeywordTok{read_ncdf}\NormalTok{(filename, }\DataTypeTok{var =} \KeywordTok{c}\NormalTok{(}\StringTok{"time"}\NormalTok{)))}

\NormalTok{variablename <-}\StringTok{ "fg10"}
\NormalTok{fg10_}\DecValTok{1}\NormalTok{ =}\StringTok{ }\KeywordTok{read_ncdf}\NormalTok{(filename, }\DataTypeTok{var =} \KeywordTok{c}\NormalTok{(variablename), }\DataTypeTok{ncsub =} \KeywordTok{cbind}\NormalTok{(}\DataTypeTok{start =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{), }\DataTypeTok{count =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{)))}
\NormalTok{fg10.units =}\StringTok{ }\KeywordTok{units}\NormalTok{(fg10_}\DecValTok{1}\OperatorTok{$}\NormalTok{fg10)}

\NormalTok{lonlat.unstack <-}\StringTok{ }\KeywordTok{expand.grid}\NormalTok{(}\DataTypeTok{lon=}\KeywordTok{as.numeric}\NormalTok{(lon}\OperatorTok{$}\NormalTok{longitude), }\DataTypeTok{lat=}\KeywordTok{as.numeric}\NormalTok{(lat}\OperatorTok{$}\NormalTok{latitude))}
...
Next code from ISD data source stored in unstacked PostgreSQL table \emph{isd\_lite\_unstack}, load twelve stations (using WHERECLAUSE to filter) and apply roughness correction factors. Correction factor values are identical because listed ISD stations are equivalent to the stations listed in the code above. An efficient management of memory is made in the code due to the use of lazy \texttt{tibble} data-frames from PostgreSQL database, see Annex \ref{dbstoring} \emph{\protect\hyperlink{dbstoring}{Database Storing}}.
...
\NormalTok{          fexpo =}\StringTok{ }\NormalTok{fc}\OperatorTok{$}\NormalTok{fexpo[fc}\OperatorTok{$}\NormalTok{station_id }\OperatorTok{==}\StringTok{ }\KeywordTok{as.integer}\NormalTok{(station_id)]  }
        \CommentTok{#Keep the record of stations without correction factors in Excel file}
          \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{length}\NormalTok{(fexpo) }\OperatorTok{==}\StringTok{ }\DecValTok{0}\NormalTok{) \{mymissing =}\StringTok{ }\KeywordTok{c}\NormalTok{(mymissing, station}\OperatorTok{$}\NormalTok{station_id[}\DecValTok{1}\NormalTok{])}
